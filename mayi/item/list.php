...
try {
    ...
    //前データ取得のSQLを生成
    $sql = "SELECT brand.chinese_name AS brand, item.id, item.on_sale, item.sample, item.japanese_name, item.chinese_name, item.weight, ship.name AS ship, shop.name AS shop, item.link, shop.link as shop_link, item.buying_price, item.selling_price,
  item.cost
FROM item 
  LEFT JOIN brand ON item.brand = brand.id 
  LEFT JOIN shop ON item.shop = shop.id 
  LEFT JOIN ship ON item.ship = ship.id 
ORDER BY item.brand,item.id ";
    ...
    echo "<table border=\"1\">\n";
    echo "<tr>\n";
    echo "<th>update</th><th>id</th><th>japanese_name</th><th>chinese_name</th><th>on_sale</th><th>weight</th><th>price_in</th>\n";
    echo "</tr>\n";
    ...
    foreach ($result as $row) {
        ...
        echo "<td>" . htmlspecialchars($row['on_sale'], ENT_QUOTES, 'UTF-8') . "</td>\n";
