...
try {
    ...
    //前データ取得のSQLを生成
    $sql = "SELECT ship_price.*, ship.name FROM ship_price
  LEFT JOIN ship ON ship_price.method = ship.id 
ORDER BY id";
    ...
    echo "<table border=\"1\">\n";
    echo "<tr>\n";
    echo "<th>name</th><th>weight_min</th><th>weight_max</th><th>price</th>\n";
    echo "</tr>\n";
    ...
    foreach ($result as $row) {
        ...
        echo "<td>" . htmlspecialchars($row['weight_min'], ENT_QUOTES, 'UTF-8') . "</td>\n";
        echo "<td>" . htmlspecialchars($row['weight_max'], ENT_QUOTES, 'UTF-8') . "</td>\n";
