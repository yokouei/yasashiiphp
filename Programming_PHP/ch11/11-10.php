...
class BookList {
	...
	function __construct($filename) {
		...
		$this->fieldType = array(
			'title' => self::FIELD_TYPE_SINGLE, 'author' => self::FIELD_TYPE_ARRAY, 'isbn' => self::FIELD_TYPE_SINGLE, 						'comment' => self::FIELD_TYPE_SINGLE,
								);
	...
	function showBook($isbn) {
		foreach ($this->records as $book) { 
			...
			echo "<p><b>{$book['title']}</b> by " . join(', ', $book['author']) . "<br />"; echo "ISBN: {$book['isbn']}<br />";
